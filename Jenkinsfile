pipeline {
    ...
    stages {
        stage('Git Checkout') {
            steps {
                checkout scmGit(branches: [[name: '*/master']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/ashinjimmy/department.git']])
        ...
        stage('SonarQube Analysis') {
            steps {
                withSonarQubeEnv('SonarQube') {
                    bat '''mvn clean verify sonar:sonar -Dsonar.projectKey=department -Dsonar.projectName='department' -Dsonar.host.url=http://localhost:9000''' //port 9000 is default for sonar
